...
<template>
  <div>
    ...
    <UDropdown :items="actions" :ui="{ width: 'w-36' }">
      <UButton color="gray" icon="i-heroicons-chevron-down" size="xs" trailing>
        Operations
      </UButton>
    </UDropdown>

    <ClientOnly>
      <UTable v-model="selectedRows" :rows="modelRows" :columns="columns" @select="select">
    ...
    <UModal v-model="isOpen">
      <UCard :ui="{ ring: '', divide: 'divide-y divide-gray-100 dark:divide-gray-800' }">
        ...
        <div>
          ...
          <ul>
            <li v-for="row in selectedRows" :key="row.name" class="font-bold">{{ row.name }}</li>
          </ul>
        </div>
